...
URL이 JSON 숫자 값을 반환하기만 하면 필요한 항목을 골라 카드로 표시할 수 있습니다. 각 항목의 이름, JSON 키, 색상, 표시 순서, 링크, 알림음과 볼륨을 개별 설정할 수 있어 간단한 상태판이나 수치 모니터링 용도로 사용할 수 있습니다.
...
- 항목별 WAV / MP3 알림음 설정
  - 값이 `0`이면 재생하지 않음
  - `0 → 1 이상`이면 알림음 재생 시작과 함께 카드 점멸
  - `1 이상 → 다른 1 이상`이면 알림음은 끊지 않고 계속 재생하고 카드만 점멸
  - `1 이상 → 0`이면 알림음을 즉시 중지하고 점멸하지 않음
  - 같은 알림 조건이 유지되는 동안 폴링마다 사운드를 재시작하지 않고 연속 재생
  - 카드의 소리 아이콘으로 일시 음소거 가능
  - 음소거 상태는 저장하지 않으며 재시작 시 초기화
- 전체 알림음 볼륨 0~100% 설정
- 항목별 알림음 볼륨 0~100% 설정
  - 실제 출력 볼륨은 `전체 볼륨 × 항목별 볼륨`으로 적용
  - 볼륨만 변경하면 재생 중인 사운드를 재시작하지 않고 즉시 음량만 변경
- 출력 사운드 장치 선택
  - `기본 장치` 또는 현재 활성화된 Windows 출력 장치 선택 가능
  - 선택한 장치가 사라지거나 사용할 수 없으면 Windows 기본 출력 장치로 자동 대체
  - 출력 장치 자체가 바뀌는 경우에만 재생 스트림을 새 장치로 재구성
- 값이 0이면 카드 배경을 약간 어둡게 표시
- 값이 1 이상으로 변경되면 약 1초간 점멸
- 항상 위(TopMost) 표시
...
`AudioDeviceId`가 빈 문자열이면 Windows 기본 출력 장치를 사용합니다. `Items` 배열의 순서가 메인 화면과 트레이 메뉴의 표시 순서가 됩니다.
